...
def _reference_clock_refusal(cfg: RunConfig, modes: list[str],
    ...
    return (
        ...
        "None on every cell, so no row can say where its clock sat relative "
        "to the GEMM its roof was measured with, and `roof_at_cell_clock_"
        "tflops` is refused for the same reason: the arm carries the "
        "fixed-roof fraction alone, with no way on the row to state the "
        "issue-rate bias in it. (DRIFT still fires; it compares a cell's own "
        "first and last samples and needs no reference.) A sweep whose rows "
        "cannot be placed against the ruler is not a conservative sweep, it "
        "is an unexamined one, and it costs the same rental as one that can.\n"
