...
| Lambda | VM (Lambda's docs: an instance can only be launched, restarted or terminated, and terminating permanently removes it) | a guest the tenant administers, so the module-parameter route in 2.2 is the tenant's to take; `scripts/setup_vm.sh` (`docs/LAMBDA.md`) picks the door (open, sudo, or the opt-in module reload) and its preflight PF5 passes only when the r3-arms probe READ back every STRICT metric as a number | **no**: the report that ncu works there is second-hand (GPU MODE); no counter has been read on a Lambda box in this repo |
| CoreWeave, Crusoe, Nebius, Together, Modal, Paperspace | mixed VM and container | **NOT VERIFIED.** I could not read their documentation in this session: the web-search budget was exhausted and I had no reliable direct URLs. Do not quote a claim about these from this page | **no** |
