...
def timing_columns(t) -> dict:
    ...
    of this arm is one; since 2026-09-09 it is a RECORD of the operating point
    and excludes nothing, and `clock_excluded` reads DRIFT alone.

    `clock_samples_of` adds the first and last under-load sample, the sample
    list and the draw. Without them the 12 DRIFT rows of the 2026-09-09 run
    say a clock moved and cannot say which way, which is the one thing a
    reader of a drifted row needs.
    """
    return {
        ...
        for g in plan.group_sizes:
            ...
            for n in (1, *plan.slope_tiles):
                ...
                try:
                    ...
                    # LEVEL sides are named as kept: every tread of this arm
                    # past the anchor is memory-shaped and boosts to 1980 MHz
                    # against the 1485 MHz reference, and the 18 LOW cells of
                    # the 2026-09-09 session were BLOCK_M=64 at GROUP_SIZE_M=1
                    # sitting at its own steady operating point.
                    ...
                    elif timed["clock_level_side"]:
                        print(f"  ^ kept (LEVEL {timed['clock_level_side']} is "
...
def run_score_measured(args) -> int:
    ...
    print(f"ceiling: {cal.describe()}")
    # RECOMPUTED FROM THE ROWS, NOT READ OUT OF THE PAYLOAD. A measure.json
    # written before 2026-09-09 carries a `clock_state` block whose rule string
    # says LOW excludes, and reprinting it would put the retired rule on a page
    # scored under the current one. The counts come from the same rows the
    # gates were scored from.
    for line in clock_state_lines(clock_state(payload["cells"])):
        print(line)
    fits, refusals, gates, table = score_measured(
