...
EXIT CODES come from `moe.bench.exit_codes`, which is the ONE table in this
repository: 0 DONE, 1 CLAIM_FAIL, 2 REFUSED, 3 INVALID, 4 ERROR. This script
used to refuse with 3 while the session driver read 3 as "measured and
invalid", so a run that had spent nothing was logged as a run whose cells must
...
#: THE NUMBERS BELOW ARE UNIFORM-ONLY AS OF 2026-09-02. They used to be the
#: POOLED figures from `docs/FINDINGS.md`, which mixed all four routing kinds
#: (uniform is 2,558 of 18,608 bf16 rows) in the same repository whose own rule,
#: `FINDINGS.md:1333-1350`, says pooling routings is INVALID for a crossing:
#: `2R/b` describes uniform routing, and under skew the layer straddles the ridge
#: so there is no single crossing to average. Rescored uniform-only with
#:
...
#: them and no gate is scored against them. deepseek-v3 under vLLM is 1.00 to
#: three places once the skewed routings come out, which the pooled 1.07 hid.
PUBLISHED_CROSSING_RATIO: dict[str, dict[str, float]] = {
    "mixtral-8x7b": {"vllm": 1.25, "sglang": 1.26},
    "qwen2-57b-a14b": {"vllm": 1.10, "sglang": 1.16},
    "deepseek-v2-lite": {"vllm": 1.03, "sglang": 1.17},
    "deepseek-v3": {"vllm": 1.00, "sglang": 1.07},
}

#: The headline over those eight measurements and its spread, same commands.
#: 1.131 +/- 0.095 against the pooled 1.149 +/- 0.069 the docs still print: the
#: centre barely moves and the DISPERSION grows 38%, because pooling four
#: routing kinds averaged four different questions into one narrow-looking
#: answer. C3's window is not derived from this number; it is printed beside the
#: measured tilt so a reader can see what is being explained.
PUBLISHED_SHIFT = 1.131
PUBLISHED_SHIFT_SD = 0.095

#: The uniform-only bf16 crossing per model, in tokens, vLLM column, same
#: commands. Used ONLY to say which token counts the confound has to be checked
#: at (C2) and to place the grid's midpoint in the plan printout, never as a
#: prediction. The pooled numbers were 454/810/922/3240; uniform-only they are
#: 26-31% EARLIER at three of the four models, which moves the bracket C2 is
#: scored over and is why they could not stay.
PUBLISHED_BF16_CROSSING: dict[str, float] = {
    "mixtral-8x7b": 313.0, "qwen2-57b-a14b": 730.0,
    "deepseek-v2-lite": 931.0, "deepseek-v3": 2925.0,
}

#: alpha, the fraction of a weight re-read that misses L2, as a function of the
#: swizzle width GROUP_SIZE_M. POOLED, UNPAIRED MEDIANS from ONE card: they are
#: exactly the `alpha against GROUP_SIZE_M` block of
#: `results/published/2026-09-01-nvidia_h200-alpha-surface-s4/SURFACE.txt`
#: (0.839 / 0.729 / 0.675 / 0.668 over n = 7 / 4 / 4 / 3 fits), rounded.
#:
#: WHAT THIS COMMENT USED TO CLAIM, AND WHY IT WAS WITHDRAWN. It said the curve
#: was "MEASURED ... monotone and saturating, and reproduced on both the H200 and
...
#:     at BM=64 goes 0.947 -> 0.699 -> 0.659 -> 0.745 and rises at the last.
#:   * The grid behind the medians is ragged (10 identifiable mixtral fits
#:     against 7 for qwen2, spread differently over G), which is the exact
#:     condition under which collapsing one lever over another is invalid.
#:
#: It is kept, as a pooled summary and labelled one, because `predicted_ms`
#: needs SOME alpha and this is the only per-swizzle curve that was measured at
#: all. Its consumers are the predicted band printed beside C3 and the
#: `--self-test` world, neither of which is a gate; nothing here is scored
#: against it. The honest reading is a level near 0.7 with a spread of about
#: 0.3 across models, tiles and cards, and no established shape in G.
ALPHA_BY_GROUP_M_POOLED: dict[int, float] = {1: 0.84, 8: 0.73, 16: 0.68, 64: 0.67}

#: The old name, kept so a reader who greps for it lands on the docstring above.
ALPHA_BY_GROUP_M = ALPHA_BY_GROUP_M_POOLED
...
#: chosen here. The three names are kept because callers use them, and every one
#: of them moved:
#:
#:   EXIT_NOT_MEASURED was 3. Three is INVALID in the one table: measured, and a
#:   VALIDITY gate failed after the money was spent. This script refused with it
#:   -- no CUDA, no fp8 path, no calibration, --dry-run -- so the session driver
#:   logged a run that had cost nothing as a run whose cells must not be quoted,
#:   and queued genuine refusals for retry. It is now REFUSED (2).
#:
#:   EXIT_GATE_FAILED was 1 for BOTH a failed VALIDITY gate and, under
#:   --fail-on-claim, a failed CLAIM gate. Those are different outcomes: a
#:   VALIDITY failure is INVALID (3), a CLAIM failure is CLAIM_FAIL (1) and is a
#:   RESULT. `exit_codes.classify` now derives the code from the gates
#:   themselves, so the name survives only as the claim-failure code.
...
CSV_COLUMNS = (
    ...
    *TIMING_CSV_COLUMNS,
    ...
    # published reports carry none of this and cannot be attributed to a
    # commit at all, which is the defect (A5) these columns close.
...
def alpha_for_group(group_m: int) -> float:
    """The POOLED L2 miss fraction at this swizzle width, interpolated.

    Interpolated in log(GROUP_SIZE_M) between the four points of
    `ALPHA_BY_GROUP_M_POOLED` and clamped outside them, because an
    ...
    between two of them and is why this is a curve and not a lookup.

    IT IS AN INTERPOLATION OF POOLED MEDIANS, NOT A MEASURED CURVE. The four
    points are one card's unpaired medians over a ragged grid; the paired series
    inside one shape at one tile are not monotone on either card and the A100's
    own medians run the other way. See `ALPHA_BY_GROUP_M_POOLED`. Nothing this
    function feeds is a gate: it supplies the band printed beside C3 and the
    `--self-test` world, both of which are corroboration or fiction.
    """
    if group_m <= 0:
        raise ValueError(f"GROUP_SIZE_M must be positive, got {group_m}")
    known = sorted(ALPHA_BY_GROUP_M_POOLED)
    if group_m <= known[0]:
        return ALPHA_BY_GROUP_M_POOLED[known[0]]
    if group_m >= known[-1]:
        return ALPHA_BY_GROUP_M_POOLED[known[-1]]
    for lo, hi in zip(known, known[1:], strict=False):
        if lo <= group_m <= hi:
            span = math.log(hi) - math.log(lo)
            frac = (math.log(group_m) - math.log(lo)) / span
            return (ALPHA_BY_GROUP_M_POOLED[lo]
                    + frac * (ALPHA_BY_GROUP_M_POOLED[hi]
                              - ALPHA_BY_GROUP_M_POOLED[lo]))
...
class Ceilings:
    ...
    measured_on: str
    path: str
    #: The SM clock the calibration's dense GEMM ran at, and where in the file
    #: it was found. `timing.time_kernel` needs it before `clock_level_ok` can
    #: mean anything: a level is relative to something, and the something is the
    #: clock the roof every cell here is scored against was measured at. None
    #: when the file carries no clock, which leaves LEVEL undetermined rather
    #: than guessed.
...
def plan_run_id(payload: dict, card: str) -> str:
    """A hash of the WHOLE plan under the CARD, built by `provenance.run_id`.
    ...
    grid, the calibration name and the lookup device are all in here.

    THE CARD WAS THE FIELD THIS ONE OMITTED, until 2026-09-02. It is not swept
    by the script, it is swept by the operator moving to another pod, and the
    results root prefers `$MOE_RESULTS_DIR` then `/workspace/results`, a network
    volume the runbook uses BECAUSE it outlives the pod. Two cards therefore
    derived one id, and the sibling sweep has the proof in the repo: one report
    filename appears under both `2026-09-01-nvidia_h200-cross-card-s3` and
    `2026-09-02-nvidia_a100_sxm4_80gb-alpha-surface-s3`. `Store` keys resume on
    (model, tokens, arm, dtype) and carries no device column, so the second card
    would find every row present, spend no GPU time, and print the first card's
    timings against its own ceilings -- 145.8 FLOP/byte against 162.8.

    THE TIMING KNOBS ARE IN IT TOO, for the same reason: `--warmup`,
    `--cell-budget-ms`, `--trials`, `--iters` and the flush state each set the
    measured milliseconds of every row, and a re-run at a different one landing
    ...
    missing card and a None knob, sorts before hashing so the id does not depend
    on the order the knobs were named in, and puts the card slug at the FRONT
    where `ls` shows it.

    THE KEYS ARE ABBREVIATED AND ORDERED, THE VALUES ARE NOT. `run_id` renders
    the knobs sorted by name into a directory name capped at 96 characters, so
    a key spelled `calibration` costs eleven of them and pushes the model off
    the end. The hash is over the full key either way, but a listing that shows
    the calibration and not the model is a listing nobody can read, so the keys
    are numbered in the order a reader wants them and abbreviated.

    `arms` is the ONE field of the plan deliberately not in the id: `ARMS` is a
    module constant with no flag behind it, so it cannot differ between two runs
    ...
    short = {"1m": payload["models"], "2t": payload["tokens"],
             ...
             "5rep": payload["reps"], "6it": payload["iters"],
             "7wm": payload["warmup"], "8bd": payload["budget"],
             "9tr": payload["trials"], "aflush": payload["flush"],
             "bseed": payload["seed"], "clookup": payload["lookup_gpu"],
             "dcal": payload["calibration"], "evllm": payload["vllm_tag"],
             "fst": payload["self_test"], "gsta": payload["self_test_alpha"],
             "hstn": payload["self_test_noise"],
             "isto": payload["self_test_overhead_ms"],
             "jsta8": payload["self_test_fp8_activations"]}
    missing = sorted(set(payload) - {"arms"} - set(_ID_FIELDS))
    if missing:
        raise ValueError(
            f"plan knob(s) {missing} are not in the run id; two settings that "
            "differ only there would share a directory, and the second would "
            "resume the first's rows and print them under its own label")
    return PV.run_id(card=card, **short)


#: The plan fields `plan_run_id` maps into the id, in plan-key terms. Kept
#: beside the mapping so the guard above is checking a list a reader can see
#: rather than a length.
_ID_FIELDS = ("models", "tokens", "dtypes", "routing", "reps", "iters",
              "warmup", "budget", "trials", "flush", "seed", "lookup_gpu",
              "calibration", "vllm_tag", "self_test", "self_test_alpha",
              "self_test_noise", "self_test_overhead_ms",
              "self_test_fp8_activations")
...
class ArmResult:
    ...
    #: NOT DETERMINED. `clock_level_ok` and `clock_drift_ok` must BOTH be read:
    #: the pre-2026-09-02 single drop-only flag is the defect they replace.
    instrument: str = ""
...
def summarise_samples(result: ArmResult, samples: list[float]) -> ArmResult:
    """Reduce a list of per-repeat p50s to the arm's headline numbers.

    THE UNIT CHANGED ON 2026-09-02 AND THE ARITHMETIC DID NOT. `samples` used to
    be individual `time_call` iterations; it is now one queue-deep p50 per
    round-robin repeat, from `timing.time_kernel`. `ms_stdev` is therefore the
    BETWEEN-REPEAT spread, which is the spread that matters to a ratio measured
    across repeats, rather than the within-cell jitter the old column reported
    and that V4 was reading as if it were run-to-run noise.
    """
...
def summarise_timings(result: ArmResult, timings: list) -> ArmResult:
    ...
    Every repeat is one `time_kernel` call, so an arm has several of them and
    the row has one of each column. The reductions are chosen so that a
    filter over the row cannot be more permissive than a filter over the
    repeats:

      * the flags are ANDed with None absorbing: False if ANY repeat was False,
...
def _fold_flag(values: list, bad: bool = False) -> bool | None:
    ...
    `bad` is the DOMINATING value, the one whose presence in any repeat decides
    the arm. It is False for the two clock flags, whose False means throttled or
    drifting, and True for `host_bound`, whose True means the interval carried
    host time. Written once with the polarity as an argument rather than twice
    with a `not` between them, because two nearly-identical folds differing by a
    negation is how a filter comes to pass a row it should have dropped.
    """
...
class Store:
    ...
    def __init__(self, path: Path, fresh: bool = False, prov=None):
        self.path = path
        #: Stamped onto every row this session writes. A row without it is a row
        #: nobody can attribute to a commit, a card or an instrument, which is
        #: what all 26 published reports are.
        self.prov = prov
    ...
    def write(self, result: ArmResult, cell: Cell, meta: dict) -> None:
        ...
        row = {
            ...
            "iters": result.iters, "trials": result.trials,
        ...
        if self.prov is not None:
            row.update(self.prov.as_columns())
...
#: The instrument, named once. RETIRED 2026-09-02: this file used to carry a
#: verbatim copy of `block_m_crossing_sweep.time_call` -- events created inside
#: the loop, a synchronise after every iteration, no flush, no clock -- and
#: compared its ratios against a roof measured queue-deep by
#: `calibrate.measure_bf16_gemm`. The audit (A7) bounded the host prefix that
#: copy exposed at ~0.18 ms per fused_experts call on the H200 pod and ~0.30 ms
#: on the A100, different per card and of the order of the cross-card effect the
#: study registered. A ratio of two arms timed the same way cancels a CONSTANT
#: prefix, but not one that scales with the number of launches, and the two
#: dtypes do not launch the same number of kernels. `timing.time_kernel` is the
...
def timing_columns(t) -> dict:
    ...
    Written as columns rather than folded into a note because the question a
    reader asks of a published ratio is "was this cell at the roof's clock, and
    was its L2 cold", and a note cannot be filtered. `None` stays None: csv
    writes it empty, and empty means NOT DETERMINED, never False.
    """
...
def _unflag(cell: str) -> bool | None:
    ...
    The distinction is the whole point of the tri-state: a cell whose clock
    could not be sampled has an undetermined level, and reading that as
    "level not ok" would drop a good cell while reading it as "ok" would keep a
    throttled one. Neither is available here, so it stays None.
    """
    text = str(cell).strip()
    if text == "":
        return None
...
def measure_cell(cell: Cell, weights_by_dtype: dict, dtypes: list[str], args,
    ...
    #: One `KernelTiming` per (arm, dtype) per repeat. The state each repeat ran
    #: in is kept, not just its p50, because a repeat that throttled has to be
    #: able to make the whole arm's row say so; see `summarise_timings`.
    records: dict[tuple[str, str], list] = {k: [] for k in usable}
            ...
            for dtype in dtypes:
                ...
                try:
                    with context(arm):
                        ...
                        # ratio here is read against was measured queue-deep.
                        # `time_kernel` is that same queue-deep loop, and it
                        # reports the clock it ran at as well as the time.
...
class Gate:
    ...
    def token(self) -> str:
        ...
        `name` reads "C4 config share" so a human table can be read; a result
        line's name field is one token by the parser's rule, so the leading
        identifier is what travels. Two gates whose first token collided would
        be indistinguishable to the driver, which is why every name here starts
        with its own V<n>/C<n>.
        """
    ...
    def result_line(self) -> str:
        ...
        Rendered by `moe.bench.exit_codes.result_line` rather than formatted
        here, so the prefix, the field order and the refusals are the same in
        every script. The human block below it is prose: the driver ignores it,
        and a line that merely contains "PASS" is not a result.
        """
        return exit_codes.result_line(
            exit_codes.VALIDITY if self.kind == "VALIDITY" else exit_codes.CLAIM,
            self.token, self.verdict, self.observed.replace("\n", " ")[:160])
...
#: time. MEASURED, not assumed: it is the range and median of
#: `timing_spread_median` over the 26 published `*.report.json` files in
#: `results/published`, which is every arm this repository has that records one
#: (min 0.0039, median 0.0077, max 0.0182 on 2026-09-02). The old convention in
#: this repo was to assume 0.5%, below the whole measured range.
...
#: every MDE it prints, named here rather than inlined so a reader can see that
#: nothing was chosen to make a gate pass.
MDE_LEVEL = 0.05
MDE_POWER = 0.80


def mde_of_ratio(spread: float, reps: int) -> float:
    """Smallest fp8/bf16 TILT this design can resolve, as a fraction.

    The design is one number per arm per cell, compared as a ratio, so the
    quantity that has to clear the noise is a difference of two log times. Each
    is a median over `reps` repeats, and a ratio inherits both spreads, hence
    the sqrt(2). With sigma imported rather than estimated within the run the
    test is a known-variance z test, which is the stricter of the two available
    forms and the only one that is evaluable at these repeat counts: a
    two-sample t at reps=3 costs 3.6 sigma against this 2.80.
...
def render_mde(args, spreads=None) -> str:
    """What this design could see, printed BEFORE the box is rented.
    ...
    resolved the difference either way. `spreads` overrides the pair of measured
    spreads, which is how the CANNOT-RESOLVE branch is planted in the tests: on
    today's corpus this design clears the effect at both ends, and a branch
    nothing can reach is a branch nobody has read. The effect C3 is about is the distance
    from a tilt of 1.0 to the published 1.131, i.e. 0.131; C4 apportions an
    excess of the same size. Both ends of the measured spread are printed,
    because the design is adequate at one and not at the other and a single
    number would hide which.
    """
    spreads = spreads or (("median", MEASURED_SPREAD_MEDIAN),
                          ("worst", MEASURED_SPREAD_MAX))
    effect = abs(PUBLISHED_SHIFT - 1.0)
    lines = ["## What this design can see (MDE)", "",
             f"Effect under test: the confounded tilt sits {effect:.3f} above "
             f"1.0 ({PUBLISHED_SHIFT:.3f} +/- {PUBLISHED_SHIFT_SD:.3f}, "
             "uniform-only).",
             f"Noise assumption: per-cell timing spread, MEASURED over the "
             f"{26} published reports; median "
             f"{MEASURED_SPREAD_MEDIAN:.2%}, worst {MEASURED_SPREAD_MAX:.2%}.",
             f"Design: {args.reps} round-robin repeats per (arm, dtype), "
             "compared as a ratio.", ""]
    for label, spread in spreads:
        ...
        lines.append(f"  at the {label:<6} spread {spread:.2%}:  MDE "
                     f"{mde:.3f}   {verdict}")
    lines += ["",
...
class NoCardToLabel(ConfoundRefusal):
    """No card could be named for this run, and none will be invented.

    Every row, the run id and the results directory carry a card. There is no
    default because a wrong one is worse than none: a laptop run labelled
    `NVIDIA H200` looks exactly like a pod run in `ls`, in the CSV and in the
    report header, and the config lookup it drives would silently derive the
    H200's tuned files for a machine that has none.
    """


def resolve_lookup_gpu(args, env: dict) -> str:
    """The device name the CONFIG LOOKUP is derived for, from `--card`,
    `--gpu-name`, or the live device. Refuses rather than defaulting.

    IT USED TO FALL BACK TO THE LITERAL "NVIDIA H200" (A5/R6). That default did
    two things at once and both were wrong. It named the plan for a card the
    machine might not be, so `--dry-run` on a laptop printed a full H200
    derivation under a heading that never said it was assumed; and, because the
    lookup device decides which of vLLM's tuned files `resolve_tile` reads, it
    silently chose the tuned half of the confound for a card that ships no tuned
    file at all. The two shapes this script defaults to have a tuned H200 file
    and nothing else does, so the default made the whole premise true by
    construction.

    `--gpu-name` still overrides, because deriving the plan for a card you are
    about to rent is a supported and useful thing to do; it is now a statement
    rather than a silent default.
    """
    name = args.gpu_name or args.card or env.get("gpu_name")
    if name:
        return str(name)
    raise NoCardToLabel(
        "no CUDA device, no --card and no --gpu-name, so there is no card to "
        "derive vLLM's config lookup for and nothing to name the run after. "
        "Pass --card 'NVIDIA H200' (or --gpu-name) to plan for a card you are "
        "about to rent; the run id and every row will say that is what you did.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--iters", type=int, default=15,
                    help="RETIRED as a timing knob on 2026-09-02 and kept in "
                         "the run id and the cost estimate. "
                         "moe.bench.timing.time_kernel sizes the iteration "
                         "count per cell from --cell-budget-ms and the warmup's "
                         "own queue-deep per-call time, which is the only "
                         "sizing that holds a trial to a duration. It stays in "
                         "the id because rows measured at a different count "
                         "exist on disk and must not be resumed into")
    ap.add_argument("--warmup", "--warmup-ms", type=float, default=300.0,
                    ...
                    help="MILLISECONDS of delivered GPU load to warm up for, "
                         ...
                         "is the wrong unit, because a 1 ms kernel needs "
                         "hundreds of calls before the clock governor reacts "
                         "and a 30 ms GEMM needs one, and this script's two "
                         "dtypes are exactly a fast kernel and a slow one")
    ap.add_argument("--cell-budget-ms", type=float, default=200.0,
                    help="target duration of ONE trial; the iteration count is "
                         "derived from it and from the warmup's own per-call "
                         "time")
    ap.add_argument("--trials", type=int, default=3,
                    help="queue-deep trials per time_kernel call; the p50 is "
                         "over iters x trials samples")
    ap.add_argument("--no-l2-flush", action="store_true",
                    help="time with L2 warm. The default FLUSHES, which is the "
                         "opposite of what this script did before 2026-09-02: "
                         "its private loop never flushed, while the roof its "
                         "ratios are read against was measured flushed, so the "
                         "two were not one instrument. A ratio of two arms "
                         "cancels a constant, and expert weights resident in a "
                         "large L2 are not a constant across two dtypes whose "
                         "weights differ in size by 2x")
    ap.add_argument("--card", default=None,
                    help="the card this run is for, as it appears in "
                         "nvidia-smi. Used for the run id and the results "
                         "directory. Off a GPU there is nothing to detect and "
                         "there is no default: a run labelled with a card it "
                         "did not run on is worse than a run with no label")
...
def _main(argv: list[str] | None = None) -> int:
    ...
    try:
        lookup_gpu = resolve_lookup_gpu(args, env)
    except NoCardToLabel as exc:
        print("\n".join([
            "REFUSED. Nothing was planned and nothing was measured.",
            f"  NoCardToLabel: {exc}"]))
        return EXIT_NOT_MEASURED
    card = args.card or env["gpu_name"] or lookup_gpu
    try:
    ...
    payload = {"models": models, "tokens": tokens, "dtypes": dtypes,
               ...
               # The three knobs that set the measured milliseconds of a row,
               # beside the retired --iters that used to be the only one here.
               "warmup": args.warmup, "budget": args.cell_budget_ms,
               "trials": args.trials, "flush": not args.no_l2_flush,
               "seed": args.seed,
               ...
               # `run_id` refuses a None, so each is rendered as a string here:
               # "none" is a value the knob took, and it has to be able to
               # differ from any number the knob could take instead.
               "self_test": str(args.self_test),
               "self_test_alpha": str(args.self_test_alpha),
               "self_test_noise": str(args.self_test_noise),
               "self_test_overhead_ms": str(args.self_test_overhead_ms),
               "self_test_fp8_activations": bool(args.self_test_fp8_activations)}
    run_id = args.run_id or plan_run_id(payload, card)
    ...
    header_lines = [
        ...
        f"{args.reps} repeats per (arm, dtype), round-robin with dtype "
        f"innermost; each repeat is one time_kernel call of {args.trials} "
    ...
    prov = PV.provenance_block(
        ...
        ridge=ceilings.ridge(BF16), ridge_source=ceilings.path,
        bandwidth=ceilings.bandwidth_bytes_s, bandwidth_source=ceilings.path,
    ...
    if args.dry_run:
        ...
        print("\n".join(["", "=" * 72,
                         ...
                         "NO `RESULT:` LINE IS PRINTED HERE. Nothing was "
                         "measured, so this run exits REFUSED,",
                         "and a REFUSED log carrying result lines would let "
                         "the driver recompute DONE",
                         "from two passing claims and disagree with the code "
                         "the process returned.", ""]))
        for g in decided:
            print(g.render(with_result=False))
        if not decided:
    ...
    (out_dir / "summary.json").write_text(json.dumps(prov.stamp({
        # `gpu_name` is the PROVENANCE block's field: the device torch reported
        # at run time, or null with a reason. This one is what the rows were
        # labelled with, which on a --self-test is the string "synthetic", and
        # the two must not be the same key: `stamp` refuses a collision, and it
        # is right to, because a synthetic label written into the provenance
        # field would make a generated page claim a card.
    ...
    # recomputes the code the process returned. Two integers meaning different
    ...
    # is not a pass: nothing on a page built from generated rows may be quoted,
    # and that is what INVALID means. The claim line below is what makes the
    # self test discriminating anyway.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        traceback.print_exc()
        print("ERROR: dtype_tile_confound crashed before it could reach a "
