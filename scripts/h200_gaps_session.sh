...
# directory (nvidia_h200-...-repeats13-s-9f91fa91): on the shared /workspace
# volume it would find all 416 of session 4's (repeat, state, tread) cells on
# disk, time nothing, and re-score them as this session's. The CARD file does
# not stop that, because it compares the card NAME and every H200 has the same
# one. With the tag a new session measures fresh and a resumed one resumes.
...
cat <<EOF
  private-mixtral-bm32  the ratio, and read it BEFORE any alpha below. It is
               ...
               $(private_duty) so they are expected to agree; a FAIL at that
               duty is a finding about this card, not a setting to change) and
               V8 (the probed alignment kernel along the ratio arms' ladder)
               beside it; the page says in as many words that nothing here
