...
arm_unpriced() { case "$1" in
  ...
  private-mixtral-bm32)
              echo "compiles and allocation, in the plan's own words, and the plan names both: the private weight build copies 16.91 GB device-to-device once, and the five-part buffer proof runs 18 extra fused_experts calls at the deepest tread after the last timed cell (3 + 3 per copy past the first, zeroing ONE copy at a time and restoring it), counted by proof_calls in the script" ;;
...
#     directory whose DEVICE file names another card's UUID. Passing it on the pod would check the plan
...
cat <<EOF
  private-mixtral-bm32  the ratio, and read it BEFORE any alpha below. It is
               ...
               V5 (the declaration: native vs shared) and V7 (the two arms'
               clocks) beside it; the page says in as many words that nothing
               here bounds a cost paid by READING the copies.
