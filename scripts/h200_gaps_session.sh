...
#     ledger word is EARNED: OPEN lands DONE, BLOCKED lands CLAIM_FAIL, and
#     REFUSE -- no ncu on PATH, or nsys without its importer --
#     exits 2 with no RESULT line and is re-attempted on every
#     resume. SINCE 2026-09-15 SO ARE THE OTHER TWO: counter_plan is the one
#     arm in `never_latches`, because it measures nothing and reads a property
#     of the POD, and --resume-latest onto a shared /workspace can be a
#     different pod. This paragraph described BLOCKED as latched, the answer
#     the session keeps, which was true of every arm when it was written.
#     `arm_closes counter_plan` says the same; an earlier version said
#     it exited DONE on BLOCKED with no RESULT line, which was true for one day
#     and contradicted `arm_closes` for five.)
...
#
#   2 HOURS  thermal, calibrate, both pin probes, private-mixtral-bm32, the three
#            rooflines, alias_ablation and bn_g16. Both payload arms with their
#            preconditions and the private-weight reading that says what their
#            alpha means, nothing else. The elasticity is in NEITHER set.
#            (Both bookings live in `arm_minutes`; this comment quotes neither.)
...
#  13 counter_plan   Probes whether a DRAM counter can be READ on this box: it
#                    launches one real CUDA kernel under ncu and reports OPEN
#                    only when a counter value comes back. A counter is the ONLY
#                    thing that turns alpha_b into a number rather than an
#                    interval. THE 2026-09-09 AND 2026-09-10 OPEN READINGS ARE
#                    RETRACTED: the probe then profiled /bin/true, which launches
#                    no kernel, so it never attempted a read and could not fail.
#                    On 2026-09-15 a rented H200 refused the read outright with
#                    ERR_NVGPUCTRPERM after two 120-minute arms had been booked
#                    on the false OPEN. It is no longer free: about 15 s, the
#                    probe child's torch import.
#  14 counter-n32-m64   THE COUNTER RUN, added 2026-09-10 on an OPEN that was
#  15 counter-n128-m64  a false positive. They are still gated on counter_plan,
#                    which now has to see a counter value to say DONE.
...
# ARMS THE LATCH DOES NOT APPLY TO, and there is exactly one. The latch is
# right for every arm that MEASURES: a finished measurement is not re-spent,
# and a CLAIM_FAIL or INVALID row is an answer the session keeps. counter_plan
# measures nothing. It reads a property of the POD -- can a counter be read
# here -- and the pod is precisely what a resume can change, because
# SESSION_PREFIX is keyed to the card MODEL, /workspace is shared between
# rentals, and LATEST_EXISTS tells the operator to --resume-latest. A latched
# row from the previous rental then gates the two 120-minute counter arms with
# an answer measured on different silicon: a stale BLOCKED retires the
# session's headline pair on a pod where the route is open, and a stale OPEN
# spends four pod-hours on one where it is not. Re-asking is about fifteen
# seconds against the 240 minutes it gates, so it is re-asked every pass.
# `ledger_arm_state` takes the LAST row for a name, so the fresh row is the one
# the gate below reads and the old row stays on the ledger as history.
# The list is a local and not a top-level assignment because this block is
# FUNCTION DEFINITIONS ONLY: tests/test_h200_gaps_session.py evals it whole.
never_latches() {
  local never="counter_plan"
  case ",$never," in (*",$1,"*) return 0 ;; esac
  return 1
}
...
arm() {
  ...
  prior="$(awk -F'\t' -v n="$name" \
    ...
    "$LEDGER" 2>/dev/null)"
  if (( DRY == 0 )) && [[ -n "$prior" ]] && never_latches "$name"; then
    note "RE-RUN $name (prior $prior in $LEDGER is NOT latched: this arm reads a property of the POD, and a resume can be on another one)"
    prior=""
  fi
...
arm_basis() { case "$1" in
  ...
  private-mixtral-bm32) echo "private_weight_reference.py --dry-run --capability 9.0 --model mixtral-8x7b --block-m 32 --treads 6 --repeats 9 --duty $(private_duty) --device-memory-gb 140 -> 'estimated GPU time 154 s at the model's own timings, excluding compiles and allocation; that includes the alignment probe's 9 s' and 'WALL CLOCK at duty $(private_duty): the ladder's 145 s of kernel time takes about 581 s, the idle gaps between bursts being the point; the probe is timed at full duty', 162 cells (6 treads x 3 arms x 9 repeats) plus the probe's 54 (one series per arm: PRIVATE's id set is probed beside SHARED's). BOOKED ON THE WALL LINE AND NOT THE KERNEL ONE, since 2026-09-22 when the arm moved to --duty $(private_duty): the idle gaps are the experiment, so the ladder's 145 s of kernel time is 581 s of pod, and the probe's 9 s run at full duty on top of it. 581 + 9 = 590 s is 9.8 min, booked 10, the ceiling of the plan's own figures; it was booked 3 off the 154 s while the arm ran at full duty. THE ROW STAYS ON THE KERNEL CLOCK because that clock is defined by what a figure LEAVES OUT and this one still leaves out the compiles and the private weight build, in the plan's own NOT IN THAT FIGURE line, so the bounded total puts it on the wall-over-model ratio like every other row that does; that over-states a row whose extra minutes are idle gaps rather than compiles, which is the direction a bound is allowed to err in. EVERY SHAPING FLAG IS ON BOTH BRANCHES except --device-memory-gb, which names a HYPOTHETICAL card for the off-GPU fit check and is deliberately absent from the measuring line: on the pod the attached device is asked instead, and passing a typed number beside a real card would check the plan against nothing. THE FIGURE IS NOT THE 2 GPU-HOURS THIS ARM WAS COMMISSIONED WITH, and the gap is not an error in either: time_kernel holds --cell-budget-ms of kernel time per trial, so 162 cells is 145 s of kernel whatever each call costs. The spare budget buys a SECOND run at another tile or another model, which is a second arm and a second run id, not more repeats of this one: the binding uncertainty here is V5's machinery bound, a systematic that does not shrink with repeats." ;;
...
arm_unpriced() { case "$1" in
  ...
  private-mixtral-bm32)
              echo "compiles and allocation, in the plan's own words, and the plan names both: the private weight build copies 25.37 GB device-to-device once (9 copies declared, 6 read: the padding that keeps the ratio arms on one alignment kernel), and the five-part buffer proof runs 24 extra fused_experts calls at the deepest tread after the last timed cell (3, plus 3 per read copy past the first, plus 2 per never-read copy, zeroing ONE copy at a time and restoring it), counted by proof_calls in the script" ;;
...
# is in both sets: it is the cheaper of the pair, it reads alpha as a ratio of
# two measured slopes, and a rental whose whole point is to reach bn_g16's
# alpha should not reach it without the one arm that says what an alpha here
# means.
...
# THIS IS THE STANDALONE DRIVER'S RERUN SET AND NOT THE NEXT BOOKING, since
# 2026-09-22: the next session is the alpha(G) chain, scripts/alpha_g_chain.sh,
# and `next_session_booking` says so before it prints this set.
...
# BLOCKED probe retires them for the whole session in about fifteen seconds;
# dtype, whose REFUSED cost nothing and was a
...
# driver's rerun can be read against a prediction instead of against a hope. A
...
rerun_expectation() { case "$1" in
  ...
  calibrate)   echo "DONE, 6/6 gates, 29 s. It read DONE on 2026-09-10 (ridge 155.9) and on 2026-09-21 (ridge 151.4, band 143.7-151.4, 663.0 TFLOP/s bf16 at 1455 MHz, triad 4378.0 GB/s; its fp8 GEMM clock fell 1395 -> 1320 MHz during the measurement, which no gate scored). Re-running it costs half a minute and stamps this session; dense bf16 has moved 7.4% across four rentals (712.3 to 663.0 TFLOP/s, against the lower), so it is not carried." ;;
  ...
  counter_plan) echo "About 15 s, which is the probe child's torch import, and it is in the set to GATE the arms below. It is the one arm a resume RE-ASKS rather than skips, because it measures nothing and reports a property of the pod. WHAT IT NOW DOES: launches one real CUDA kernel under ncu and reads dram__bytes_read.sum back, so P1 PASS means a counter was READ and not merely that ncu started. THE 2026-09-09 AND 2026-09-10 P1 PASSES ARE RETRACTED: the probe then profiled /bin/true, which launches no CUDA kernel, so ncu attached, found nothing, printed 'No kernels were profiled' and exited 0 without ever attempting a read; both published payloads carry that warning beside the word OPEN. On 2026-09-15 a rented H200 booked two 120-minute arms on that OPEN and both died in 35 seconds with ERR_NVGPUCTRPERM, on a pod holding neither CAP_SYS_ADMIN nor CAP_PERFMON. ITS WORD IS THE POD'S, DONE on OPEN and CLAIM_FAIL on BLOCKED, and this row no longer predicts one: it predicted BLOCKED until 2026-09-23, and the committed record is two refused rentals, 2026-08-25 (profiles/q2_kernel_names.txt) and 2026-09-15, beside the 2026-09-09 and 2026-09-10 pods that were never asked, which is two pods and not the platform. BLOCKED here retires BOTH counter arms for this whole session at a cost of fifteen seconds, which is the reason it runs first." ;;
...
next_session_booking() {
  ...
  printf '\n==== %s ====\n' "THE STANDALONE DRIVER'S RERUN, AND WHY IT IS --new"
  echo "  THE NEXT SESSION IS THE alpha(G) CHAIN, bash scripts/alpha_g_chain.sh"
  echo "  (docs/POD_RUNBOOK.md, \"The alpha(G) chain\"). This block is the"
  echo "  standalone path: the rerun set of this driver alone, for a rental that"
  echo "  books the driver and not the chain. The chain runs this driver for its"
  echo "  preconditions, so the block is in that step's log too, as the driver's"
  echo "  record and not as the chain's plan."
  echo ""
...
# THE DUTY THE PRIVATE-WEIGHT REFERENCE RUNS AT, IN ONE PLACE, for the reason
# the counter pair's BLOCK_M is: both of its arm lines, `arm_basis`,
# `arm_offgpu_gates` and `arm_closes` read this function, so the duty cannot be
# changed on one of them and left standing on another. DESIGN DECISION 15 as
# the owner decided it on 2026-09-22: R3 runs at 0.25 wherever it runs. The
# evidence is session 4's clock arm, the same native kernel timed by the same
# `time_duty` R3 imports: at duty 0.5 the clock still tracked board power
# (-1.09 MHz/W over 1882-1965 MHz, 13 of 78 cells at treads 1-6 drifting,
# 16.7%), and at 0.25 every tread's median sat at 1965 MHz and no cell drifted.
# The script's OWN default stays 1.0 and this file does not ask it to move:
# duty is one of its DESIGN_KEYS with a default of 1.0, so moving the default
# would make every full-duty report on disk read as a different design and
# break --replicate-of against session 4's runs.
private_duty() { echo 0.25; }
...
arm_closes() { case "$1" in
  ...
  private-mixtral-bm32) echo "ALPHA AS A RATIO OF TWO MEASURED SLOPES, with no assumed bandwidth and no fitted intercept anywhere in it, which is the one thing the 2026-09-10 reading says this apparatus has never had. BOTH SLOPES ARE FITTED OVER TREADS 2 AND DEEPER (the arm's DESIGN DECISION 16, the window R1's claim reads), and so are C2's rate and V5's declaration fit; V4 checks and V0's tread floor counts exactly those treads, and V8 prices a probed alignment step at its leverage over them; the same fits over every tread, tread 1 included, and tread 1's distance from the claim's lines are printed beside the claim and gate nothing (on session 5's pages the one-tile call sat 0.157-0.164 ms above the line through treads 2-6 at G >= 4); V6 still reads tread 1, where shared and private are one call, and V7 still scores every tread. Every alpha in this study divides a time by an ASSUMED rate or by an extrapolation back to n=0, and that is why the same six cells give 0.9794 under one parameterisation and 0.5977 under another against a quoted sd of 0.0113. This arm runs the ladder THREE TIMES at one geometry, with copy c of expert e at expert slot e x n_decl + c (EXPERT-FIRST): SHARED (E x n_decl experts declared at EVERY tread, every M-tile routed to copy 0), PRIVATE (the same declaration and tensors, M-tile j routed to copy j, so reuse is impossible BY CONSTRUCTION and the slope IS the alpha=1 reference), and NATIVE (the study's own call, E declared over a strided view of copy 0: the same bytes at the same addresses, which is the control V5 reads to tie the matched ratio to the path the study fits). n_decl is the deepest tread's copy count padded until E x n_decl clears vLLM's small-batch expert bound, which is 9 copies and 72 slots at the booked ladder: 6 read, 3 filled and never read. SHARED and PRIVATE share the sorted-id buffer, the launch grid, the dead launches and the tile ORDER, and differ only in the addresses tiles read; an earlier copy-first layout that declared E x n at tread n broke both, adding dead launches to the private slope alone and running its tiles in a different order, and order is the lever this study measured moving the per-M-tile cost by 30-48%. V7 requires the two arms' clocks to agree at every tread. AT FULL DUTY a pod that cannot lock its clock fails that by construction whenever the arms draw different power (session 4: four ratio pages at full duty, every one INVALID, the private arm's clock 2-18% below the shared arm's: 2% on mixtral-8x7b at G=1 in both of its runs, 12% on qwen2-57b-a14b at G=1, 18% on mixtral-8x7b at G=16), so THIS ARM RUNS AT --duty $(private_duty): every cell is timed as ~40 ms bursts with idle gaps between them, each gap sized from the burst it follows so every arm achieves the requested duty (session 5 sized them from a 20 ms full-duty reading, and the arms of one run achieved duties 2.5-6.9% of 0.25 apart), which takes board power off the cap without changing a byte the kernel moves. The duty is the owner's DESIGN DECISION 15 and session 4's clock arm is its evidence: at duty 0.5 the native kernel's clock still tracked board power (-1.09 MHz/W over 1882-1965 MHz, 16.7% of cells at treads 1-6 drifting), and at 0.25 every tread's median sat at 1965 MHz with no drifting cell. V7 is therefore EXPECTED to hold here, not guaranteed: the private arm is inferred to draw more power than the native kernel the clock arm measured (on session 4's 700 W cap it held 1425 MHz where the shared arm held up to 1740 at G=16; its power at duty 0.25 has not been measured). A V7 FAIL at $(private_duty) means that duty is not yet flat for that arm on this card; the page names a lower duty; the alpha(G) chain skips the G's later seeds and prints the follow-up command. This driver runs seed 0 alone, so on its path the follow-up is by hand: a new design key and its own runs at the lower duty, outside this ledger, read with --read on the laptop, and never a replicate of this run. The duty is in the run id and is a design key, so a replicate at another duty is refused. With --clock-elasticity set to R1's per-M-tile claim (clock_elasticity's elasticity.value with its interval, fitted over treads 2 and deeper, from a report whose elasticity block carries claim_min_tread, and not session 4's elasticity.value, which is the pooled per-call reading) and --clock-elasticity-source naming that report, the key and the commit, the page PRINTS a clock-corrected ratio beside the raw one and scores nothing by it. AND THE ALIGNMENT KERNEL IS SEPARATED, THREE WAYS: vLLM's moe_align_block_size switches kernel at ids < 1024 and experts <= 64 (cited to v0.27.1's csrc/libtorch_stable/moe/moe_align_sum_kernels.cu), which for mixtral at BM=32 falls between treads 3 and 4, inside the fit; the ratio arms declare 9 copies (72 experts) so both take one kernel throughout and the census REFUSES a plan where they do not; a probe times the alignment op alone along the ladder, once per arm (NATIVE's declaration, and SHARED's and PRIVATE's id sets at the ratio arms'), before a weight is allocated, and V8 refuses the design on the MEASUREMENT if the ratio arms' series carries a step worth more than 0.01 of the ratio (the step is judged at the Student-t quantile of its own standard error, widened for the fact that its split was chosen by minimum residual, and bounded over both ratio arms with no premise that the step is common), skipping the sweep on a FAIL ONLY: an UNKNOWN is a statement about the probe, not the design, so the ladder still runs and the page latches INVALID with every other gate's number on it. The probe times the op under a CUDA graph (PROBE_CALLS_PER_REPLAY calls per replay), so its cells are GPU time and NATIVE's switch beside them confirms the cited source or bounds its cost, never withholding V8; on the EAGER fallback (capture refused, named on the page), which an H200 makes host-bound, NATIVE's own switch at the census tread is the positive control: resolved there, the probe has shown it sees a kernel switch through the host cost and a flat ratio series earns PASS; and NATIVE, which keeps the study's E=8 and so its switch, has its difference from SHARED fitted with a step term, so V5 reads the declaration's per-tile cost with the step out and the step is printed with an interval. Nothing here bounds a cost paid by READING the copies, and the plan page says so. slope(shared)/slope(private) is alpha as a traffic fraction with no assumed RATE in it, and it is a traffic fraction on the further assumption that both arms deliver the same bytes per second, which the page registers rather than hides. NO KERNEL WAS WRITTEN: vLLM fused_moe reads its expert index PER M-TILE and addresses experts through stride(0), so a private copy per tile is a relabelling of topk_ids plus a wider w1/w2. WHAT IT SETTLES, either way: a ratio in ALPHA_BAND confirms the refit as a traffic fraction; a ratio near 1.0 says the whole weight set is re-read and the model SHAPE was right where its coefficient was not identifiable; a ratio under 0.35 says most of the per-M-tile cost is issue and latency, the traffic model is the wrong KIND of model, and the study's negative result becomes a positive one. The arm PRINTS which, from a partition registered before the run. BY-PRODUCT: slope(private) read as a rate is the delivered weight-read bandwidth of the kernel under test, which is the denominator every published w is divided by, measured. IT RUNS AFTER THE PIN PROBES because it forces its tile through the same override hook they validate, and AFTER calibrate because C2 and V4 are scored against this card own ridge and triad. BM=32 AND NOT 64, and that is the design, not taste: at 64 a world of alpha=0.10 puts the shared ladder over 95% of the roof from tread 3, so V4 would void the page in the one outcome worth the most, and the script REFUSES that configuration at plan time with V4's fixed-roof floor printed. That is where V4 would void the page, not what timing resolves: V4 tests only that no fitted tread reaches the fixed compute roof, which is not memory bound (session 5 passed it at 20-25% of that roof while the shared per-M-tile cost sat on a clock-scaled on-chip floor), and the DISCRIMINATION FLOOR, the alpha under which a shared ladder on that floor reads the same, is read off each run's own shared ladder and printed with its fits." ;;
  ...
  counter_plan) echo "Whether a DRAM counter is reachable here, and it now GATES arms 14 and 15, which are the counter pair. A counter is the only route to alpha_b as a number rather than an interval, and this records which way THIS pod fell. NO WORD IS PREDICTED FOR IT: this line said the route was open until 2026-09-15 and that it was probably blocked from then until 2026-09-23. Two rented H200s have refused the read with ERR_NVGPUCTRPERM, on 2026-08-25 (profiles/q2_kernel_names.txt) and on 2026-09-15, none has returned a counter, and two pods are a record, not the platform. The 2026-09-09 and 2026-09-10 RunPod H200s both read OPEN, but that probe profiled /bin/true, which launches no CUDA kernel, so it never attempted a counter read and could not have failed. The probe now launches a real kernel and OPEN means a counter value came back. READ ITS RESULT LINES, AND THE LEDGER WORD IS EARNED: since 2026-09-03 scripts/dram_counter_route.py --probe scores one gate per verdict, prints one RESULT line each, and exits through exit_codes.classify over the same gates, so OPEN and BLOCKED land as the words the table gives them and this driver's second opinion reads the page rather than finding it blank. (Until that day it exited 0 for OPEN and BLOCKED alike with no RESULT line, and before that 3 for everything but OPEN, which filed BLOCKED as INVALID; both halves are fixed.) BLOCKED is the ANSWER, not a broken instrument: a rented pod that cannot reach a DRAM counter is a fact about the pod, recorded so the session that follows can read which way it fell. IT IS RE-ASKED ON EVERY PASS, unlike every other finished arm: counter_plan is the one name in never_latches, because a counter route is a property of the POD and --resume-latest onto a shared /workspace can be a different pod. Fifteen seconds against the 240 minutes it gates is the cheapest question in the session, and a latched verdict from the previous rental is the expensive one." ;;
...
arm_offgpu_gates() { case "$1" in
  ...
  private-mixtral-bm32) echo "scripts/private_weight_reference.py --self-test refit|no-reuse|issue-bound|aliased|machinery|compute-bound|noisy-identity|clock-split|alignment-step|host-bound-probe|host-bound-controlled|ratio-path-split|ratio-step-over-budget|ratio-step-under-budget|private-step-alone|clock-split-elastic|graph-probe-unresolved|over-allocated|holes|ragged|faster-than-its-ruler|private-k-reuse  (twenty-two planted worlds, eleven RESULT lines each, and they SEPARATE by exit code: refit, alignment-step, host-bound-controlled, ratio-step-under-budget and graph-probe-unresolved 0 DONE (host-bound-controlled is the EAGER-fallback case, a host-bound probe rescued by NATIVE's switch as the positive control, graph-probe-unresolved is the graph-timed probe that resolved no NATIVE step, a BOUND on the switch with V8 PASSING on the GPU-timed ratio series, and ratio-step-under-budget plants a real ratio-arm step worth 0.7 of V8's budget, the nearest a PASS sits to V8's edge); no-reuse and issue-bound 1 CLAIM_FAIL on C1 with the outcome NAMED on the line; faster-than-its-ruler 1 CLAIM_FAIL on C2 AND NOT 3, because it plants every arm delivering 25% over the card's own calibration and C1 does not move with it -- both terms of the ratio moved together -- so it is the one world in this table that refutes the delivered-rate denominator rather than the apparatus; private-k-reuse 1 CLAIM_FAIL on C1 and C2 with every validity gate PASS, the PRIVATE-K world (it plants --private-copies 2 and runs at it with no flag given): in the order the kernel runs them each expert's tiles read copy 0 in one run and copy 1 in the next, each tile after a run's first re-reading its predecessor's copy at the shared alpha, so its denominator sits on the shared slope, its ratio is not alpha and its implied rate beats the ceiling by reuse, and both gates say so in their own words; and the remaining THIRTEEN 3 INVALID on the validity gates each is planted to break (one of them, host-bound-probe, on a gate that reads UNKNOWN rather than FAIL, which counts against a VALIDITY gate the same way, one, ratio-step-over-budget, on the same V8 step at 1.5 budgets, the nearest a FAIL sits to V8's edge, one, private-step-alone, on that same step planted in PRIVATE's id set ALONE, which is the asymmetry the pair bound exists for and the common-step bound it replaced could not express, and one, clock-split-elastic, on V7 with a planted elasticity, where the page PRINTS a clock-corrected ratio that recovers the refit world exactly and scores the raw one anyway), which for 'ragged' is TWO of them, V0 on the counts and V1 on the tread sets, and the world registers both. Every gate FAIL branch is reachable from some world and tests/test_private_weight_reference.py COUNTS that rather than listing it). Then --dry-run --duty $(private_duty) --device-memory-gb 140 for the memory arithmetic, the depth table with V4's fixed-roof floor (the discrimination floor is read off a run's own ladder and is not on the plan) and the wall clock at the duty the arm runs at, and --dry-run --block-m=64 --device-memory-gb 140, which REFUSES and is the check that chose the default tile. THE EQUALS FORM IS DELIBERATE: this driver pins the counter pair's tile in counter_block_m() and tests/test_h200_gaps_session.py forbids a second copy of that literal anywhere in this file, so the tall tile is named here in the form argparse accepts and that grep does not match." ;;
  roofline-n64-g1|roofline-n256-g16|roofline-n256-g32)
              echo "scripts/bm128_roofline.py --self-test --fail-on-gate  (seven planted worlds, measuring nothing on any box, exit 0 required)" ;;
  ...
  elasticity-m32-n64-g16) echo "scripts/clock_elasticity.py --self-test  (TWELVE planted worlds, SEVEN of them REFUSALS, five VALIDITY RESULT lines and exit 0; the run prints both counts off its own world list, so read them there and not from this line). THE FIVE WORLDS THAT CARRY IT: 'one-clock' plants four duty states that all settled at the same clock and V1 refuses it, which is the refusal this whole arm turns on -- three states at one clock measure nothing; 'level-both-sides' plants a LEVEL HIGH row, a LEVEL LOW row and a DRIFTING one in one state and asserts both LEVEL rows are KEPT, because at a duty cycle below 1.0 the boosted rows ARE the experiment and a scorer that filtered on LEVEL would drop the signal; 'straddling' is the only world in which C1 can FAIL, so the design-resolution gate is proven reachable rather than assumed; 'clock-carries' at 0.60 is the only world in which C2 can FAIL, which is the reading that RETRACTS C3's direction; and 'slower-at-speed' plants elasticity -0.35, a call that got SLOWER as the clock rose at a byte-identical kernel, which RAW-STANDS is OPEN to below zero -- C1 and C2 both read PASS and the page would print 'the clock is not what is wrong with alpha' over a run whose clock channel is broken. V7 is the only gate that can see it and this is the world that makes its FAIL branch reachable. S2 and S3 are the estimator gates and they are the point: S2 asks that a planted elasticity comes back, S3 that a DIFFERENT planted elasticity comes back different, so a scorer reporting its prior cannot pass. Also scripts/clock_elasticity.py --dry-run --model mixtral-8x7b --dtype bf16 --group-m 16 --treads 8 --duty 1.0 0.5 0.25 0.1 --repeats 13 --burst-ms 40 --target-ms 200 --trials 3 --warm-ms 200 --settle-seconds 10 for the three registered bands, the design arithmetic that CHOOSES the V1 threshold, and the priced wall clock." ;;
...
note "           ablation and bn_g16 -- with their preconditions, plus the $(arm_minutes private-mixtral-bm32)-minute"
note "           private-weight reference that says what their alpha MEANS, and"
note "           nothing else. The $(arm_minutes elasticity-m32-n64-g16)-minute clock elasticity is in NEITHER set:"
...
#     the cheaper booking of the two in `arm_minutes`, and that one can spend
#     its whole booking and still exit INVALID when the duty states fail to
#     separate.
#
#     --duty "$(private_duty)" IS ON BOTH BRANCHES, since 2026-09-22, and it
#     is DESIGN DECISION 15 as the owner decided it: this arm and the alpha(G)
#     chain run R3 at one duty. Session 4's four ratio pages were INVALID on
#     V7 at full duty, the power cap boosting whichever arm read fewer bytes:
#     the private arm's clock ran 2-18% below the shared arm's (2% on
#     mixtral-8x7b at G=1 in both of its runs, 12% on qwen2-57b-a14b at G=1,
#     18% on mixtral-8x7b at G=16). `private_duty` says why the duty is 0.25
#     and not 0.5. Two consequences travel with it. THE RUN ID CARRIES THE
#     DUTY (duty enters it whenever it is not 1.0), so this arm's directory is
#     never a full-duty one and a hand-run seed 1 has to pass the same --duty
#     to be admitted by --replicate-of, where duty is a design key. AND THE
#     WALL CLOCK IS ABOUT 1/duty TIMES THE KERNEL TIME: the plan prints both,
#     and `arm_minutes` books the wall one (see `arm_basis`).
#
...
#
#     --session-tag IS ON BOTH BRANCHES and it is what makes --new mean new.
#     $RESULTS is scoped to the card, not the session, so without the tag a
#     second session on the same card resumed the first one's cells, timed
#     nothing, and reported them as its own; the script also refuses a
#     directory whose DEVICE file names another card's UUID. THE TAG IS THE
#     ONE PART OF THE RUN ID A DRY SESSION CANNOT PREVIEW: a dry invocation
#     opens its own session directory, so its tag, and with it the "WRITES
#     TO" path on the dry page, differ from the measuring session's. Every
#     other knob in the id is identical on both branches: every flag that
#     shapes the LADDER -- the model, the tile, the depth, the repeats, the
#     duty -- is on both, so the dry run previews the pod's own run id and not
#     a different sweep.
# --------------------------------------------------------------------------
...
if (( DRY )); then
  arm private-mixtral-bm32 "$PY_BASE" "$REPO/scripts/private_weight_reference.py" --dry-run \
      ...
      --treads 6 --repeats 9 --duty "$(private_duty)" \
      --session-tag "$(basename "$SESSION")" --device-memory-gb 140
else
  arm private-mixtral-bm32 "$PY_VLLM" "$REPO/scripts/private_weight_reference.py" \
      ...
      --duty "$(private_duty)" --session-tag "$(basename "$SESSION")"
fi
...
# other, and forty minutes that can end in INVALID do not go ahead of a cheaper
# arm that measures alpha directly. Sweeping the admissible elasticity moves pooled EXA alpha_b
...
#
# THIS IS THE STANDALONE ARM, AND scripts/alpha_g_chain.sh IS NOT A COPY OF IT.
# The chain runs clock_elasticity once per G of the ratio ladder, at its own
# duty states, as the regime word beside each G's ratio; nothing in this
# driver re-runs that ladder. This arm keeps, deliberately, the design
# session 4 ran: ONE swizzle, G=16 (the one every published alpha was fitted
# at, and the g16 in the arm's name), and FOUR states from the cap to the boost
# ceiling, 1.0 0.5 0.25 0.1, the widest clock span one byte-identical kernel
# reaches on a card that refuses -lgc. So a run here is a second card's reading
# of session 4's exact design, comparable cell for cell, and it is what an
# operator gets from the driver alone. --group-m 16 is ON THE LINE rather than
# left to the script's default, so the G the name promises is the G the line
# asks for; at 16 it does not move the run id.
#
# --session-tag IS ON BOTH BRANCHES, since 2026-09-22, for the reason 0c's is.
# $RESULTS is keyed on the card and not the session, and without the tag this
# line's run id on an H200 is byte-identical to session 4's published
# directory (nvidia_h200-...-repeats13-s-9f91fa91), and on the shared
# /workspace volume that directory holds all 416 of session 4's (repeat,
# state, tread) cells. Before the arm had a DEVICE guard it would have timed
# nothing and re-scored them as this session's, since the CARD file compares
# the card NAME and every H200 has the same one. Since 2026-09-22 the guard
# REFUSES a directory holding cells.csv and no DEVICE file, as session 4's
# does, or another card's UUID, so without the tag the arm would now refuse
# rather than re-score; the tag is what lets a new session measure fresh and a
# resumed one resume.
# THE TAG IS THE ONE PART OF THE RUN ID A DRY SESSION CANNOT PREVIEW, as in 0c:
# the dry page names its own session directory, not the pod's.
if (( DRY )); then
  ...
  arm elasticity-m32-n64-g16 "$PY_BASE" "$REPO/scripts/clock_elasticity.py" --dry-run \
      --model mixtral-8x7b --dtype bf16 --group-m 16 --treads 8 \
      --duty 1.0 0.5 0.25 0.1 --repeats 13 --burst-ms 40 --target-ms 200 \
      --trials 3 --warm-ms 200 --settle-seconds 10 \
      --session-tag "$(basename "$SESSION")"
else
  ...
  # clock carries enough of the per-M-tile cost (treads 2 and deeper, the
  # claim clock_elasticity gates) that C3's direction is
  ...
  case "$ELASTICITY_ROW" in
    DONE)       note "   alpha      the interval fell wholly below 0.25: at this cell what"
                note "              one more M-tile adds (treads 2 and deeper) is TRAFFIC,"
                note "              not issue rate; tread 1 is printed beside it. The"
...
# probe costs about fifteen seconds and answers exactly the question that
# licenses these arms' four pod-hours, so a BLOCKED or REFUSED verdict has to
# stop the spend rather than be read off the page afterwards. It is skipped
# rather than refused, because nothing is broken when a host declines counter
# collection.
#
# THE ROW IT READS IS THIS POD'S, AND THIS COMMENT ASSERTED THAT WITHOUT
# ARRANGING IT UNTIL 2026-09-15. It said "not a remembered result from another
# rental: a counter route is a property of the pod, and the pod is what changes
# between sessions", and both halves of that are true while the conclusion did
# not follow. SESSION_PREFIX is `gaps-$CARD-`, keyed to the card MODEL and to
# nothing that identifies a pod; /workspace is a shared network volume this
# driver already warns about at MOE_RESULTS_DIR; a plain re-invocation that
# finds a session refuses with LATEST_EXISTS and tells the operator to
# --resume-latest. So a replacement H200, told to resume, would re-open the
# previous pod's directory, and `arm` would skip counter_plan as already
# CLAIM_FAIL -- retiring the session's headline pair on a box nobody asked.
# counter_plan is therefore in `never_latches`: it does not MEASURE, it reads a
# property of the pod, and re-asking costs fifteen seconds against the 240
# minutes it gates.
counter_arm() {
  local name="$1" bn="$2"
  if (( DRY )); then
    ...
    skip_arm "$name" \
      "counter_plan is $(ledger_arm_state counter_plan), not DONE, so no counter was READ on this box in this session. DONE is the OPEN verdict and since 2026-09-15 it means ncu returned a number for dram__bytes_read.sum off a real kernel, not merely that ncu started; CLAIM_FAIL is BLOCKED, which is a fact about the pod and not a broken instrument; REFUSED is no ncu on PATH or no CUDA device for the probe kernel, either of which says the image or the interpreter is wrong rather than the pod. This arm is 120 wall minutes and it can measure nothing without the route, so it is skipped rather than spent. Read $LOGS/counter_plan.log for which of the three it was. THE ROW IS THIS POD'S: counter_plan is in never_latches and re-probes on every pass, so a resume onto a REPLACEMENT card overwrites the previous rental's verdict rather than inheriting it. If the verdict is BLOCKED and you want the route, that is a provider-side ask (--cap-add=PERFMON first, --cap-add=SYS_ADMIN second) on a NEW rental, not a re-run here."
...
cat <<EOF
  private-mixtral-bm32  the ratio, and read it BEFORE any alpha below. It is
               ...
               so reuse is impossible by construction. Both slopes are fitted
               over treads 2 and deeper, and so are C2 and V5; V4 and V0's
               floor count those treads and V8 prices its step over them; the
               every-tread fits are printed beside them and gate nothing. The
               page NAMES the world
               it landed in from a partition registered before the run --
               ISSUE-AND-LATENCY, below/at/above the refit band, or NO-REUSE --
               and a C1 FAIL is one of those names and not a broken arm; C1
               UNKNOWN means the point and interval disagree on a world. Read
               V5 (the declaration: native vs shared, with native's alignment
               step fitted out), V7 (the two arms' clocks, run at --duty
               $(private_duty), each idle gap sized from the burst it follows,
               so they are expected to agree; a V7 FAIL at
               $(private_duty) means that duty is not yet flat for that arm on
               this card; the page names a lower duty; the alpha(G) chain
               skips the G's later seeds and prints the follow-up command, and
               here the follow-up is by hand, a new design key and its own
               runs, read with --read on the laptop) and V8 (the probed
               alignment kernel along the ratio arms' ladder) beside it; the
               page says in as many words that nothing here bounds a cost paid
               by READING the copies.
...
# THE STANDALONE DRIVER'S RERUN, printed only where a session was actually run
# (the next session is the alpha(G) chain, and the block says so first): a
# --dry-run has no ledger of results to book a rerun against, and printing a
# rerun plan under a plan would be a second schedule on the same page.
if (( DRY == 0 )); then
  next_session_booking
fi
...
cat <<EOF
  ...
  prints no such block. That is this driver's own path: the next session is
  the alpha(G) chain, scripts/alpha_g_chain.sh. Deleting rows out of a ledger
  by hand is the other way back and it edits the record of what was spent;
  --new does not.
  ONE ARM IS EXEMPT AND ONLY ONE: counter_plan re-probes on every pass however
  its last row read, because it measures nothing and reports a property of the
  POD, which a resume onto a shared /workspace can change under it. Its old row
  stays on the ledger; the gate reads the newest.
  Copy it off before releasing the pod:
      tar czf /workspace/exfil-gaps-$CARD.tar.gz \
          -C "$(dirname "$SESSION")" "$(basename "$SESSION")" \
          -C "$(dirname "$RESULTS")" "$(basename "$RESULTS")"
  THE RESULTS ROOT IS IN THAT LINE AND WAS NOT. $SESSION holds the ledger and
  the logs; the per-arm cells.csv, report.json, DEVICE file and triton-cache
  live under $RESULTS, which is scoped to the card and outlives the session.
  A tarball of $SESSION alone carries the RESULT lines and not the rows they
  were scored from.
EOF
