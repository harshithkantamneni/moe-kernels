...
arm_offgpu_gates() { case "$1" in
  ...
  elasticity-m32-n64-g16) echo "scripts/clock_elasticity.py --self-test  (THIRTEEN planted worlds, EIGHT of them REFUSALS, five VALIDITY RESULT lines and exit 0; the run prints both counts off its own world list, so read them there and not from this line). THE SIX WORLDS THAT CARRY IT: 'one-clock' plants four duty states that all settled at the same clock and V1 refuses it, which is the refusal this whole arm turns on -- three states at one clock measure nothing; 'level-both-sides' plants a LEVEL HIGH row, a LEVEL LOW row and a DRIFTING one in one state and asserts both LEVEL rows are KEPT, because at a duty cycle below 1.0 the boosted rows ARE the experiment and a scorer that filtered on LEVEL would drop the signal; 'straddling' is the only world in which C1 can FAIL, so the design-resolution gate is proven reachable rather than assumed; 'clock-carries' at 0.60 is the only world in which C2 can FAIL, which is the reading that RETRACTS C3's direction; and 'slower-at-speed' plants elasticity -0.35, a call that got SLOWER as the clock rose at a byte-identical kernel, which RAW-STANDS is OPEN to below zero -- C1 and C2 both read PASS and the page would print 'the clock is not what is wrong with alpha' over a run whose clock channel is broken. V7 is the only gate that can see it and this is the world that makes its FAIL branch reachable; and 'thin-cell-sag' plants one row that sagged inside its bursts in a cell DRIFT thinned to 5 of 13 repeats, inside V5's one-row budget and under half the cell, which still carries the cell's median in 11% of the interval's bootstrap draws and so can set an interval edge: V5's bootstrap share clause fails it, and it is the world that makes that branch reachable. S2 and S3 are the estimator gates and they are the point: S2 asks that a planted elasticity comes back, S3 that a DIFFERENT planted elasticity comes back different, so a scorer reporting its prior cannot pass. Also scripts/clock_elasticity.py --dry-run --model mixtral-8x7b --dtype bf16 --group-m 16 --treads 8 --duty 1.0 0.5 0.25 0.1 --repeats 13 --burst-ms 40 --target-ms 200 --trials 3 --warm-ms 200 --settle-seconds 10 for the three registered bands, the design arithmetic that CHOOSES the V1 threshold, and the priced wall clock." ;;
